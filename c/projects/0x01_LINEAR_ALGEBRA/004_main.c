...
void run_test(Matrix *m, bool copy, VecOrientation o, const char *label) {
  ...
  // }
    printf("Orthogonalized Matrix (ort(M)):\n");
    print_matrix(m);
    // safe_destroy(&m);
}
...
int main (void) {
  ...
  run_test(m1, false, VERTICAL, "Test 1: Full Rank 3x3 Matrix");
  safe_destroy(&m1);
  ...
  run_test(m2, false, VERTICAL, "Test 2: Rectangular 4x2 Matrix");
  safe_destroy(&m2);
  ...
  // run_test(m3, false, VERTICAL, "Test 3: Rank-Deficient Matrix");
  // safe_destroy(&m3);
  ...
  run_test(m4, false, VERTICAL, "Test 4: Symmetric Matrix");
  safe_destroy(&m4);
  ...
  run_test(m5, false, VERTICAL, "Test 5: Already Orthonormal Matrix");
  safe_destroy(&m5);

  Matrix *m6 = create_matrix(1000, 1000, 0);
  for (int i = 1; i <= 1000; i++) {
    mat_set(m6, i, i, (float)i); // diagonal entries: 1,2,3,...,100
  }

  run_test(m6, false, VERTICAL, "Test 6: Large 1000x1000 Diagonal Matrix");
  safe_destroy(&m6);
